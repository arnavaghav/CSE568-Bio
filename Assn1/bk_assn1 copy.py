...
def evaluate(individual):
    ...
    all_combinations = {pair: set() for pair in pairs}
    
    for row in included_rows:
        for (i, j) in pairs:
            all_combinations[(i, j)].add((configuration[row][i], configuration[row][j]))
    
    covered = sum(len(all_combinations[pair]) == V**T for pair in pairs)
    # Update fitness function to apply a larger penalty for uncovered pairs
    uncovered_pairs = len(pairs) * V**T - covered
    return (len(included_rows) + uncovered_pairs * 5,)
