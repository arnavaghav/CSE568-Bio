...
def evaluate(individual):
    ...
    column_combinations = combinations(range(K), T)
    all_combinations = {comb: set() for comb in column_combinations}
    
    for row in included_rows:
        for comb in column_combinations:
            all_combinations[comb].add(tuple(configuration[row][i] for i in comb))
    
    covered = sum(len(all_combinations[comb]) == V**T for comb in column_combinations)
    uncovered_combinations = len(list(column_combinations)) - covered  # Fix here
    return (len(included_rows) + uncovered_combinations * 5,)
